# ------------------------------------------------------------------------- #
# HS-SCPV2-FPN: HS-FPN with a bidirectional Semantic Context Prior branch.
#
# This file intentionally does not modify hs_fpn.py.  It keeps the original
# HFP/SDP structure and adds:
#   - SCPV2PoolingContext + LightweightSemanticHead
#   - SCPV2SemanticFusion (low-frequency semantic embed guides HFP)
#   - SCPV2DetailGate (P1/P2 high-frequency detail guides semantic embed)
#   - HS_SCPV2_FPN neck registered as a separate module
# ------------------------------------------------------------------------- #
...
__all__ = [
    'HS_SCPV2_FPN', 'HFP_SCPV2', 'SCPV2Branch',
    'SCPV2PoolingContext', 'SCPV2DetailGate', 'SCPV2SemanticFusion',
    'PoolingLowFreqExtractor', 'LowFreqExtractor',
...
# SCPV2 branch.
#
# Compared with V1, SCPV2 removes the DCT low-pass filter from the semantic
# branch.  The semantic branch uses pooled context to build a low-frequency
# embedding, and keeps semantic embedding separate from semantic logits.
# ------------------------------------------------------------------------- #
class SCPV2PoolingContext(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=dict(
                     ...
                     distribution='uniform')):
        super().__init__(init_cfg)
        compressed_ch = in_channels // compress_ratio
        self.pool = nn.AdaptiveAvgPool2d(pool_size)
        self.compress = ConvModule(
            in_channels, compressed_ch, 1, bias=False)
        self.refine = nn.Sequential(
            ConvModule(
                ...
                3,
                padding=1,
                groups=compressed_ch,
                bias=False),
            ConvModule(compressed_ch, compressed_ch, 1, bias=False))
    ...
    def forward(self, x):
        ...
        return self.refine(self.compress(z))


class PoolingLowFreqExtractor(SCPV2PoolingContext):
    """Backward-compatible alias for the SCPV2 pooling context branch."""
    pass


class LowFreqExtractor(SCPV2PoolingContext):
    """Backward-compatible name for the SCPV2 pooling low-frequency branch."""
...
class LowFreqExtractorNoDCT(SCPV2PoolingContext):
    """Backward-compatible alias kept for old configs."""
    pass
...
class SCPV2DetailGate(BaseModule):
    ...
    def __init__(self,
        ...
        self.gate = nn.Sequential(
            ConvModule(feat_channels, embed_channels, 1, bias=False),
            nn.Conv2d(
                embed_channels,
                embed_channels,
                3,
                padding=1,
                groups=embed_channels,
                bias=False),
            _make_group_norm(embed_channels, 16),
            nn.ReLU(inplace=True),
            nn.Conv2d(embed_channels, embed_channels, 1),
            nn.Sigmoid())

    def forward(self, high_feat):
        return self.gate(high_feat)


class HighFreqDetailGate(SCPV2DetailGate):
    """Compatibility alias for the phase-B SCPV2 detail gate."""
    pass


class SCPV2Branch(BaseModule):
    ...
    def __init__(self,
                 ...
                 isdct=True,
                 pool_size=(8, 8),
                 enable_detail_gate=True,
                 detach_detail=True,
                 high_to_low_beta_init=0.0,
                 detail_scale_init=None,
                 init_cfg=dict(
                     type='Xavier', layer='Conv2d',
                     distribution='uniform')):
        super().__init__(init_cfg)
        compressed_ch = in_channels // 4
        self.detach_detail = detach_detail
        if detail_scale_init is not None:
            high_to_low_beta_init = detail_scale_init
        self.lfe = SCPV2PoolingContext(
            in_channels, compress_ratio=4, pool_size=pool_size)
        self.detail_gate = SCPV2DetailGate(
            feat_channels=in_channels,
            embed_channels=compressed_ch) if enable_detail_gate else None
        self.high_to_low_beta = nn.Parameter(
            torch.tensor(float(high_to_low_beta_init)))
        self.semantic_head = LightweightSemanticHead(
            compressed_ch, num_classes=num_classes)

    def forward(self, x, guide_feat=None):
        z = self.lfe(x)
        if self.detail_gate is not None and guide_feat is not None:
            detail_src = guide_feat.detach() if self.detach_detail else guide_feat
            detail_gate = self.detail_gate(detail_src)
            z = z * (1.0 + self.high_to_low_beta * detail_gate)
        return z, self.semantic_head(z)


class SCP(SCPV2Branch):
    """Compatibility alias for older SCP configs.

    Internally this is the SCPV2 phase-B branch: pooled semantic context plus
    optional P1/P2 high-frequency detail guidance.
    """
    pass


class SCPV2SemanticFusion(BaseModule):
    """Gated semantic embedding fusion.

    Unlike V1, this module consumes semantic embeddings instead of raw
    semantic logits, and injects the low-frequency context through a learnable
    gated residual path.
    """
    ...
    def __init__(self,
                 ...
                 init_cfg=dict(
                     ...
                     distribution='uniform')):
        super().__init__(init_cfg)
        embed_channels = embed_channels or max(1, feat_channels // 4)
        self.attn_dim = attn_dim
        self.conv_q = nn.Sequential(
            ConvModule(feat_channels, attn_dim, 1, padding=0, bias=False),
            _make_group_norm(attn_dim, 32))
        self.conv_k = nn.Sequential(
            ConvModule(embed_channels, attn_dim, 1, padding=0, bias=False),
            _make_group_norm(attn_dim, 32))
        ...
        self.fusion_scale = nn.Parameter(
            torch.tensor(float(fusion_scale_init)))
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, hfp_feat, semantic_embed, patch_size):
        _, _, h, w = hfp_feat.shape
        ph, pw = patch_size

        q = self.conv_q(hfp_feat)
        k = self.conv_k(semantic_embed)
        v = self.conv_v(semantic_embed)
        ...
        gate = self.gate(torch.cat([hfp_feat, semantic_embed], dim=1))
        return hfp_feat + self.fusion_scale * gate * delta


class SemanticCrossAttention(SCPV2SemanticFusion):
    """Compatibility alias for SCPV2 gated semantic embedding fusion."""
    pass


class HFP_SCPV2(BaseModule):
    ...
    def __init__(self,
                 ...
                 detail_scale_init=None,
                 fusion_scale_init=0.0,
                 semantic_fusion_source='embed',
                 init_cfg=dict(
                     type='Xavier', layer='Conv2d',
                     distribution='uniform')):
        super().__init__(init_cfg)
        assert semantic_fusion_source in ('embed', 'logits')
        self.semantic_fusion_source = semantic_fusion_source
        self.hfp = HFP(
            in_channels, ratio=ratio, patch=patch, isdct=isdct)
        self.scp = SCPV2Branch(
            in_channels,
            ratio=ratio,
            num_classes=num_classes,
            isdct=isdct,
            pool_size=pool_size,
            enable_detail_gate=enable_detail_gate,
            detach_detail=detach_detail,
            high_to_low_beta_init=high_to_low_beta_init,
            detail_scale_init=detail_scale_init)
        self.cross_attn = SCPV2SemanticFusion(
            feat_channels=in_channels,
            num_classes=num_classes,
            attn_dim=attn_dim,
            embed_channels=(num_classes if semantic_fusion_source == 'logits'
                            else in_channels // 4),
            fusion_scale_init=fusion_scale_init)

    def forward(self, x, patch_size):
        hfp_out = self.hfp(x)
        semantic_embed, semantic_logits = self.scp(x, guide_feat=hfp_out)
        fusion_context = (semantic_logits if self.semantic_fusion_source
                          == 'logits' else semantic_embed)
        fused = self.cross_attn(hfp_out, fusion_context, patch_size)
        return fused, semantic_logits


class HFP_SCP(HFP_SCPV2):
    """Compatibility alias for the SCPV2 phase-B HFP/SCP block."""
    pass


@MODELS.register_module()
class HS_SCPV2_FPN(BaseModule):
    ...
    def __init__(self,
                 ...
                 enable_high_to_low_detail=None,
                 detach_detail=True,
                 high_to_low_beta_init=0.0,
                 detail_scale_init=None,
                 fusion_scale_init=0.0,
        ...
        assert semantic_fusion_source in ('embed', 'logits')
        self.semantic_fusion_source = semantic_fusion_source
        assert len(scp_pool_sizes) == 4
        if enable_high_to_low_detail is not None:
            enable_p12_detail_gate = enable_high_to_low_detail
        if detail_scale_init is not None:
            high_to_low_beta_init = detail_scale_init
        ...
        self.SelfAttn_p4 = HFP_SCPV2(
            ...
            isdct=False,
            pool_size=scp_pool_sizes[3],
            enable_detail_gate=False,
            detach_detail=detach_detail,
            high_to_low_beta_init=high_to_low_beta_init,
            fusion_scale_init=fusion_scale_init,
            semantic_fusion_source=semantic_fusion_source)
        self.SelfAttn_p3 = HFP_SCPV2(
            out_channels,
            ratio=None,
            num_classes=num_semantic_classes,
            patch=(8, 8),
            attn_dim=scp_attn_dim,
            isdct=False,
            pool_size=scp_pool_sizes[2],
            enable_detail_gate=False,
            detach_detail=detach_detail,
            high_to_low_beta_init=high_to_low_beta_init,
            fusion_scale_init=fusion_scale_init,
            semantic_fusion_source=semantic_fusion_source)
        self.SelfAttn_p2 = HFP_SCPV2(
            out_channels,
            ratio=ratio,
            num_classes=num_semantic_classes,
            patch=(8, 8),
            attn_dim=scp_attn_dim,
            isdct=True,
            pool_size=scp_pool_sizes[1],
            enable_detail_gate=enable_p12_detail_gate,
            detach_detail=detach_detail,
            high_to_low_beta_init=high_to_low_beta_init,
            fusion_scale_init=fusion_scale_init,
            semantic_fusion_source=semantic_fusion_source)
        self.SelfAttn_p1 = HFP_SCPV2(
            out_channels,
            ratio=ratio,
            num_classes=num_semantic_classes,
            patch=(16, 16),
    ...
    def forward(self, inputs):
        ...
        if used_backbone_levels == 4:
            ...
            raise AssertionError(
                'HS_SCPV2_FPN expects 3 or 4 input feature levels, '
