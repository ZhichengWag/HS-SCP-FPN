...
class DctSpatialInteraction(BaseModule):
    ...
    def forward(self, x):
        ...
        with torch.amp.autocast(device_type=x.device.type, enabled=False):
            idct = DCT.dct_2d(x.float(), norm='ortho')
            weight = self._compute_weight(h0, w0, self.ratio).to(
                device=x.device, dtype=idct.dtype)
            weight = weight.view(1, h0, w0).expand_as(idct)
            dct = idct * weight # filter out low-frequency features
            dct_ = DCT.idct_2d(dct, norm='ortho').to(dtype=input_dtype) # generate spatial mask
        return x * dct_
...
class DctChannelInteraction(BaseModule):
    ...
    def forward(self, x):
        ...
        with torch.amp.autocast(device_type=x.device.type, enabled=False):
            ...
            dct = idct * weight # filter out low-frequency features
            dct_ = DCT.idct_2d(dct, norm='ortho').to(dtype=input_dtype)
...
class HS_FPN(BaseModule):
    ...
    def forward(self, inputs):
        ...
        if used_backbone_levels == 4:
            ...
            laterals[2] = self.SelfAttn_p4(laterals[2])
            laterals[1] = self.CrossAtten_p4_p3(self.SelfAttn_p3(laterals[1]), self.fpn_upsample(laterals[2]), [h, w])
            laterals[0] = self.CrossAtten_p3_p2(self.SelfAttn_p2(laterals[0]), self.fpn_upsample(laterals[1]), [h, w])
        else:
            raise AssertionError(
                'HS_FPN expects 3 or 4 input feature levels, '
